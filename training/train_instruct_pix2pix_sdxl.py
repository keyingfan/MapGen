...
from datasets.features.image import Image as DatasetsImage
...
def main():
    ...
    collect_x_data, collect_y_data, collect_prompt = [], [], []
    for scale in ["10", "15", "25"]:
        prompt_text = f"generalize to scale {scale}"
        train_path = os.path.join(args.dataset_name, f"gebc-geb{scale}")
        x_data = np.load(os.path.join(train_path, "geb_train.npy"))
        y_data = np.load(os.path.join(train_path, f"geb{scale}_train.npy"))
       

        mask = np.sum(x_data, axis=(1, 2, 3)) > 128 * 128 * 0.25
        x_data = x_data[mask]
        y_data = y_data[mask]
        

        collect_x_data.append(x_data)
        collect_y_data.append(y_data)
        
        collect_prompt.extend([prompt_text] * len(x_data))

    x_data = np.concatenate(collect_x_data, axis=0)
    y_data = np.concatenate(collect_y_data, axis=0)
   

    start_num_rows = 0
    end_num_rows = len(x_data)
    num_rows = end_num_rows - start_num_rows
    
    
    ind_list = [i for i in range(len(x_data))]
    random.Random(42).shuffle(ind_list)
    x_data = x_data[ind_list]
    y_data = y_data[ind_list]
    
    collect_prompt = np.array(collect_prompt)[ind_list]

    collect_prompt[start_num_rows:end_num_rows], len(collect_prompt[start_num_rows:end_num_rows])
    ...
    data_dict = {"input_image": np.uint8(x_data[start_num_rows:end_num_rows]*255.0).squeeze(1),
                 "edited_image": np.uint8(y_data[start_num_rows:end_num_rows]*255.0).squeeze(1),
                 "edit_prompt": collect_prompt[start_num_rows:end_num_rows],

                }

    data_feaures = {'edited_image': DatasetsImage(decode=True, id=None),
                    'input_image': DatasetsImage(decode=True, id=None),
                    'edit_prompt': Value(dtype='string', id=None),
                    
                    }

    data_feaures = Features(data_feaures)
    dataset = {}
    dataset["train"] = Dataset.from_dict(data_dict, features=data_feaures)
    ...
    src = batch["original_pixel_values"][1]
    tar = batch["edited_pixel_values"][1]

    # 修改：将input_ids改为prompt_embeds和add_text_embeds
    print(batch["original_pixel_values"].shape, 
        batch["edited_pixel_values"].shape, 
        batch['prompt_embeds'].shape, 
        batch['add_text_embeds'].shape)

    # 可选：如果你想查看文本嵌入的详细信息
    print(f"prompt_embeds shape: {batch['prompt_embeds'].shape}")
    print(f"add_text_embeds shape: {batch['add_text_embeds'].shape}")

    plt.figure()
    plt.imshow(src.permute(1,2,0), cmap="jet")
    plt.figure()
    plt.imshow(tar.permute(1,2,0), cmap="jet")
    plt.figure()
    plt.imshow((tar - src).permute(1,2,0)[:,:,0], cmap="jet")

    print(src.max(), src.min(), tar.max(), tar.min())

    # 可选：查看文本嵌入的数值范围
    print(f"prompt_embeds range: {batch['prompt_embeds'].min()} to {batch['prompt_embeds'].max()}")
    print(f"add_text_embeds range: {batch['add_text_embeds'].min()} to {batch['add_text_embeds'].max()}")
